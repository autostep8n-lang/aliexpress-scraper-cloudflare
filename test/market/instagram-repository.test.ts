import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import type { Env } from "../../src/env";
import { upsertInstagramSignals } from "../../src/supabase/repository";
import type { InstagramObservationRow } from "../../src/market/types";
import { createMockPostgrest, type MockPostgrest, type RecordedRequest } from "../helpers/postgrest-mock";

const SUPABASE_URL = "https://example.supabase.co";
const SECRET_KEY = "test-secret-key";

const INSTAGRAM_SOURCE = {
  id: "66666666-6666-6666-6666-666666666666",
  slug: "instagram",
  name: "Instagram",
  kind: "platform",
};

const INSTAGRAM_CONFLICT = "source_id,keyword";

function configuredEnv(): Env {
  return { SUPABASE_URL, SUPABASE_SECRET_KEY: SECRET_KEY } as Env;
}

function row(overrides: Partial<InstagramObservationRow> = {}): InstagramObservationRow {
  return {
    source_id: null,
    keyword: "smart watch",
    hashtag: "smartwatch",
    result_limit: 25,
    media_count: 6,
    top_media_count: 4,
    recent_media_count: 3,
    total_likes: 5300,
    total_comments: 439,
    total_engagement: 5739,
    avg_likes: 883.33,
    avg_engagement: 956.5,
    top_media_id: "media_top3",
    top_media_caption: "This budget smart watch reels reel is getting attention #smartwatch",
    captured_at: "2026-03-01T00:00:00.000Z",
    metadata: { media: [] },
    ...overrides,
  };
}

function requestsTo(server: MockPostgrest, method: string, path: string): RecordedRequest[] {
  return server.requests.filter((request) => request.method === method && request.url.includes(path));
}

describe("upsertInstagramSignals", () => {
  let server: MockPostgrest;

  beforeEach(() => {
    server = createMockPostgrest();
    vi.stubGlobal("fetch", server.fetch);
  });

  afterEach(() => {
    vi.unstubAllGlobals();
    vi.restoreAllMocks();
  });

  it("returns credentials_missing without touching the network when Supabase is unconfigured", async () => {
    const fetchMock = vi.fn(server.fetch);
    vi.stubGlobal("fetch", fetchMock);

    const result = await upsertInstagramSignals({} as Env, [row()]);

    expect(result).toEqual({ status: "credentials_missing" });
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it("returns updated with an empty payload without touching the network for no rows", async () => {
    const fetchMock = vi.fn(server.fetch);
    vi.stubGlobal("fetch", fetchMock);

    const result = await upsertInstagramSignals(configuredEnv(), []);

    expect(result).toEqual({ status: "updated", data: [] });
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it("creates the instagram source and inserts rows, backfilling source_id", async () => {
    const result = await upsertInstagramSignals(configuredEnv(), [row(), row({ keyword: "phone" })]);

    expect(result.status).toBe("created");
    if (result.status !== "created") return;

    const source = server.store.sources.find((entry) => entry.slug === "instagram");
    expect(source).toBeDefined();
    expect(source).toMatchObject({ slug: "instagram", name: "Instagram", kind: "api" });

    expect(server.store.instagram_signals).toHaveLength(2);
    expect(server.store.instagram_signals.every((entry) => entry.source_id === source?.id)).toBe(true);
    expect(result.data).toHaveLength(2);
    expect(result.data.every((entry) => entry.source_id === source?.id)).toBe(true);
  });

  it("reuses an existing instagram source without duplicating it", async () => {
    server.seed("sources", [INSTAGRAM_SOURCE]);

    const result = await upsertInstagramSignals(configuredEnv(), [row()]);

    expect(result.status).toBe("created");
    expect(server.store.sources.filter((entry) => entry.slug === "instagram")).toHaveLength(1);
    expect(server.store.instagram_signals[0].source_id).toBe(INSTAGRAM_SOURCE.id);
  });

  it("replaces a snapshot instead of duplicating on a re-collect of the same keyword", async () => {
    const first = await upsertInstagramSignals(configuredEnv(), [row()]);
    expect(first.status).toBe("created");

    const second = await upsertInstagramSignals(configuredEnv(), [row({ media_count: 99 })]);
    expect(second.status).toBe("updated");
    expect(server.store.instagram_signals).toHaveLength(1);
    expect(server.store.instagram_signals[0].media_count).toBe(99);
  });

  it("keeps separate snapshots for different keywords", async () => {
    await upsertInstagramSignals(configuredEnv(), [row()]);
    await upsertInstagramSignals(configuredEnv(), [row({ keyword: "phone", hashtag: "phone", media_count: 5 })]);

    expect(server.store.instagram_signals).toHaveLength(2);
  });

  it("targets the dedup key with on_conflict on the upsert request", async () => {
    await upsertInstagramSignals(configuredEnv(), [row()]);

    const post = requestsTo(server, "POST", "/rest/v1/instagram_signals")[0];
    expect(new URL(post.url).searchParams.get("on_conflict")).toBe(INSTAGRAM_CONFLICT);
  });

  it("sends the normalized observation payload with a resolved source_id", async () => {
    await upsertInstagramSignals(configuredEnv(), [row()]);

    const post = requestsTo(server, "POST", "/rest/v1/instagram_signals")[0];
    const payload = post.body as Array<Record<string, unknown>>;
    expect(Array.isArray(payload)).toBe(true);
    expect(payload).toHaveLength(1);
    expect(payload[0]).toMatchObject({
      keyword: "smart watch",
      hashtag: "smartwatch",
      result_limit: 25,
      media_count: 6,
      top_media_count: 4,
      recent_media_count: 3,
      total_likes: 5300,
      total_comments: 439,
      total_engagement: 5739,
      avg_likes: 883.33,
      avg_engagement: 956.5,
      top_media_id: "media_top3",
      top_media_caption: "This budget smart watch reels reel is getting attention #smartwatch",
      captured_at: "2026-03-01T00:00:00.000Z",
      metadata: { media: [] },
    });
    expect(payload[0].source_id).toBe(server.store.sources[0].id);
  });

  it("returns error source_lookup_failed when the source lookup fails", async () => {
    server.override("GET", "/rest/v1/sources", 500, {
      code: "PGRST301",
      message: "Database error",
    });

    const result = await upsertInstagramSignals(configuredEnv(), [row()]);

    expect(result.status).toBe("error");
    expect((result as { code?: string }).code).toBe("source_lookup_failed");
  });

  it("returns error source_create_failed when the source upsert fails", async () => {
    server.override("POST", "/rest/v1/sources", 500, {
      code: "23502",
      message: "null value in column violates not-null constraint",
    });

    const result = await upsertInstagramSignals(configuredEnv(), [row()]);

    expect(result.status).toBe("error");
    expect((result as { code?: string }).code).toBe("source_create_failed");
  });

  it("returns error instagram_signals_upsert_failed when the database rejects the write", async () => {
    server.seed("sources", [INSTAGRAM_SOURCE]);
    server.override("POST", "/rest/v1/instagram_signals", 400, {
      code: "23505",
      message: "duplicate key value violates unique constraint",
    });

    const result = await upsertInstagramSignals(configuredEnv(), [row()]);

    expect(result.status).toBe("error");
    expect((result as { code?: string }).code).toBe("instagram_signals_upsert_failed");
  });

  it("never leaks credentials into request URLs", async () => {
    await upsertInstagramSignals(configuredEnv(), [row()]);

    for (const request of server.requests) {
      expect(request.url).not.toContain(SECRET_KEY);
      expect(request.headers.get("Authorization")).toBe(`Bearer ${SECRET_KEY}`);
    }
  });
});
